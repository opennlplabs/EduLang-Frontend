...
// NEED TO CHANGE STORAGE  AND USED AWAIT AND JOITAI REFACTOR AND ACCOUNT FOR NO REPONSE
...
export const getCloudBooks = async languageFilter => {
  ...
  snapshot.forEach(doc => {
    ...
    if (
      (data.language == languageFilter && !titlesLang.includes(data.title)) ||
...
export const addBookLocally = async (
  title,
  description,
  language,
  book,
...
export async function addToFav(item) {
    ...
    return null
    //   var favBooks = JSON.parse(await Storage.getItem({key: 'favBooks'}));;
...
//   var data = JSON.parse(await Storage.getItem({key: 'data'}));;
...
//     .doc(item.title + ' ' + uid.toString());;

//   await document.set({
//     title: item.title,
//     language: item.language,
//     description: item.description,
//     lenPages: Object.keys(item.book).length,
//     source: item.source,
//   });;
