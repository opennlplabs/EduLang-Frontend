...
# To enable accurate and helpful autocomplete during developing :)
...
def changeImage(page):
    ...
    for orig_paragraph in page.paragraphs:
        ...
        left_para = deepcopy(orig_paragraph).apply_offset(
            offset_x=-orig_paragraph.paragraphBox.w)
        right_para = deepcopy(orig_paragraph).apply_offset(
            offset_x=orig_paragraph.paragraphBox.w)
        top_para = deepcopy(orig_paragraph).apply_offset(
            offset_y=-orig_paragraph.paragraphBox.h)
        bottom_para = deepcopy(orig_paragraph).apply_offset(
            offset_y=orig_paragraph.paragraphBox.h)
        ...
        fontpath = os.path.join(
            os.getcwd(), "src", "Fonts", "ArialUnicodeMs.ttf")
        font_size = 50
        font = ImageFont.truetype(
            fontpath, font_size, layout_engine=ImageFont.LAYOUT_RAQM)
        ...
        img[bb_best.y: bb_best.y + bb_best.h, bb_best.x: bb_best.x +
            bb_best.w, 0] = best_para.dominant_color[0]
        img[bb_best.y: bb_best.y + bb_best.h, bb_best.x: bb_best.x +
            bb_best.w, 1] = best_para.dominant_color[1]
        img[bb_best.y: bb_best.y + bb_best.h, bb_best.x: bb_best.x +
            bb_best.w, 2] = best_para.dominant_color[2]
